# README: visión — Kvasir multiclase (`dl/vision_baseline_kvasir/`)

Documentación del **prototipo solo Kvasir v2**: clasificación en **4 clases** a partir de imágenes preprocesadas, distinto del baseline binario CVC+Kvasir en `dl/vision_baseline/` (ver `dl/vision_baseline/README.md` y `data/resumen_diagnostico_imagen.md`).
...
4. **Manifest de entrenamiento (deduplicado por MD5 del EDA):**  
   `dl/vision_baseline_kvasir/generar_manifest.py`  
...
5. **Splits train/val/test (por `group_id`, estratificado por clase):**  
   `dl/vision_baseline_kvasir/crear_splits.py`  
...
## Módulo `dl/vision_baseline_kvasir/`
...
uv run python dl/vision_baseline_kvasir/evaluar.py --ultimo-run

# 5) Analisis ampliado (mismas imagenes, metricas y figuras adicionales)
uv run python dl/vision_baseline_kvasir/analisis_evaluacion.py --ultimo-run --split test
```
...
Cada ejecución crea `dl/vision_baseline_kvasir/runs/resnet18_YYYYMMDD_HHMMSS/` con, entre otros:
...
> Para replicar un entrenamiento al estilo antiguo (fijo 10 épocas, sin early stopping): `uv run python dl/vision_baseline_kvasir/entrenar.py --sin-early-stopping --epocas 10`.
...
**Run de referencia (histórico, baseline suave de documentación detallada por época en val):** `dl/vision_baseline_kvasir/runs/resnet18_20260422_121534/`
