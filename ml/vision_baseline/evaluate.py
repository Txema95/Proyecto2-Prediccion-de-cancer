...
try:
    from ml.vision_baseline.dataset import DatasetColonoscopiaBinario
except ModuleNotFoundError:
    # Permite ejecutar el script desde ml/vision_baseline sin depender del paquete "ml".
...
def parsear_argumentos() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--ultimo-run",
        action="store_true",
        help="Usa ml/vision_baseline/runs/<carpeta_mas_reciente>/best_checkpoint.pt",
    )
...
def resolver_ruta_checkpoint(raiz: Path, checkpoint: Path | None, ultimo_run: bool) -> Path:
    if ultimo_run:
        carpeta_runs = raiz / "ml" / "vision_baseline" / "runs"
        ...
        if not candidatos:
            raise FileNotFoundError(
                ...
                f"Ejecuta antes: uv run python ml/vision_baseline/train.py ..."
            )
    ...
    if "<run>" in texto_ruta or "<run>" in texto_arg:
        raise FileNotFoundError(
            ...
            f"Sustituye <run> por el nombre real de la carpeta bajo ml/vision_baseline/runs/ "
            f"(p. ej. resnet50_20260420_123456) o ejecuta:\n"
            f"  uv run python ml/vision_baseline/evaluate.py --ultimo-run --modelo resnet50"
        )
    return ruta
