# README: ML por imagen — `ml/vision_baseline/`
...
`train.py` y `evaluate.py` importan esta clase (`from ml.vision_baseline.dataset import ...` o `from dataset import ...` si se ejecuta desde la subcarpeta).
...
- `--output-dir`: `ml/vision_baseline/runs/`

Cada ejecución crea una carpeta `ml/vision_baseline/runs/<modelo>_YYYYMMDD_HHMMSS/` con, entre otros:
...
```bash
uv run python ml/vision_baseline/train.py
uv run python ml/vision_baseline/train.py --modelo mobilenet_v2 --epocas 20 --batch-size 32 --metrica-checkpoint val_f1
```
...
- `--ultimo-run`: elige el `best_checkpoint.pt` más reciente por fecha de modificación bajo `ml/vision_baseline/runs/`.
- `--checkpoint RUTA`: ruta relativa a la raíz del proyecto o absoluta al `.pt`.

**Ejemplos**

```bash
uv run python ml/vision_baseline/evaluate.py --ultimo-run --modelo resnet50
uv run python ml/vision_baseline/evaluate.py --checkpoint ml/vision_baseline/runs/resnet50_20260420_165518/best_checkpoint.pt --modelo resnet50
```
...
2. `uv run python ml/vision_baseline/train.py` [opciones].
3. `uv run python ml/vision_baseline/evaluate.py --ultimo-run --modelo <mismo_que_train>`.
